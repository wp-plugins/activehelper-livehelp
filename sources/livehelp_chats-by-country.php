...
function activeHelper_liveHelp_chatsByCountry()
{
	global $wpdb, $activeHelper_liveHelp;

	$actions = array(
		'list' => 'activeHelper_liveHelp_chatsByCountryList'
	);
...
function activeHelper_liveHelp_chatsByCountryPost()
{
	$actions = array(
		'export' => 'activeHelper_liveHelp_chatsByCountryExportPost',
		'list' => 'activeHelper_liveHelp_chatsByCountryListPost'
	);
...
function activeHelper_liveHelp_chatsByCountryExportPost()
{
	global $wpdb, $activeHelper_liveHelp;
	...
	$chatsByCountryList = $wpdb->get_results("
		SELECT
	...
	$export = '"Country","Chats","Raiting"';

	if (!empty($chatsByCountryList))
		foreach ($chatsByCountryList as $chatsByCountry)
		{
			$export .= "\n" . '"' .
				$chatsByCountry['country'] . '","' .
				$chatsByCountry['chats'] . '","' .
				$chatsByCountry['rating'] .
			'"';
		}

	header("Cache-Control: public");
	header("Content-Description: File Transfer");
	header("Content-Disposition: attachment; filename=LiveHelp_ChatsByCountry_" . date("Y-m-d", $timeStart) . "~" . date("Y-m-d", $timeEnd) . ".csv");
...
function activeHelper_liveHelp_chatsByCountryListPost()
{
	global $wpdb, $activeHelper_liveHelp;
...
function activeHelper_liveHelp_chatsByCountryList()
{
	global $wpdb, $activeHelper_liveHelp;
	...
	echo '
<div class="wrap"><form action="admin.php?page=' . strtolower('activeHelper_liveHelp_chatsByCountry') . '" method="post" accept-charset="utf-8" id="activeHelper_liveHelp_form">
	<h2 style="padding-right: 0;">
		LiveHelp » ' . __('Chats by country', 'activehelper_livehelp') . '
		<a class="button add-new-h2" href="admin.php?page=' . strtolower('activeHelper_liveHelp_chatsByCountry') . '&amp;action=export&amp;type=csv" id="activeHelper_liveHelp_export">' . __('export to CSV', 'activehelper_livehelp') . '</a>
	</h2>';
	...
	<div class="metabox-holder" style="padding-bottom: 10px;">
		...
		<table cellspacing="0" class="wp-list-table widefat fixed">
			<thead>
				<tr>
					<th style="width: 100%" class="manage-column" scope="col">
			...
			else
			{
				...
				<tr valign="top" class="' . ($alternate ? 'alternate' : '') . ' format-default">
					<td style="padding: 1ex;" class="post-title page-title column-title">
						<strong>' . $chatsByCountry['country'] . '</strong>
					</td>
